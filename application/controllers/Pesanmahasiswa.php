...
class Pesanmahasiswa extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model(array('Mahasiswa_model' => 'mahasiswa', 'Bimbingan_model' => 'penelitian', 'Dosen_model' => 'dosen', 'Proposal_model' => 'proposal', 'Grouppesan_model' => 'grouppesan', 'Pesan_model' => 'pesan'));
	...
	public function index()
	{
		...
		$this->templatemahasiswa->backend('pesan_v', $data);
	}
		...
		if (!empty($query)) 
		{
			foreach ($query as $row) {
				if ($row->Name1 == $id) {
					...
					$maha = $this->dosen->where('NPP', $row->Name2)->get();
					$nama = $maha->Nama;
				...
				if ($row->Name2 == $id) {
					...
					$maha = $this->dosen->where('NPP', $row->Name1)->get();
					$nama = $maha->Nama;
					$pendek = explode(" ", $nama);
					if(count($pendek) > 1)
					{
							...
							<div class="d-flex justify-content-start mb-2 mr-5" id="chat">
	                            <div class="card col-8 border-0 px-2 py-1 mr-5">
	                                <div class="row">
	                                    ...
	                                    <div class="col-6">
	                                        <p class="mx-2">Update Proposal Terbaru '.$proposal2->NamaBAB.' Proposal</p>
	                                        <div class="col-12 my-2">
	                                            <a href='.site_url('pesanmahasiswa/download/'.$proposal2->NamaFile.'').' class="btn btn-danger btn-sm col-12">Download<i class="fas fa-file-pdf ml-2"></i></a>
	                                        </div>
	                                    </div>
	                                </div>
	                                <p class="h7 text-right text-gray mb-n1">' . format_indo($row->created_at) . '</p>
	                            ...
	                            <div class="card bg-chat col-8 border-0 px-2 py-1 ml-5">
	                                <div class="row">
	                                    ...
	                                    <div class="col-6">
	                                        ...
	                                        <div class="col-12 my-2">
	                                            <a href='.site_url('kaprodi/pesan/download/'.$proposal2->NamaFile.'').' class="btn btn-danger btn-sm col-12">Download<i class="fas fa-file-pdf ml-2"></i></a>
	                                        </div>
	                                    </div>
	                                </div>
	                                <p class="h7 text-right text-gray mb-n1">' . format_indo($row->created_at) . '</p>
