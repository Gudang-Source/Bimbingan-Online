...
class Profile extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->library(array('ion_auth', 'form_validation', 'templatekaprodi', 'pdf'));
	...
	public function index()
	{
		...
		$this->templatekaprodi->backend('profile_v', $data);
	}
	...
	public function get_profile()
	{
		...
		$dosen = $this->dosen->where('NPP', $id)->get();
		$users = $this->users->where('username', $id)->get();

		if(!empty($dosen) && !empty($users))
		{
			$data = array(
				'Nama' => $dosen->Nama,
				'NPP' => $dosen->NPP,
